...
function Post(props) {
  ...
  const url = `https://nickrosh.com/${props.slug}`
  const date = new Date(props.date).toISOString()
  const image = props.image
    ? `https://nickrosh.com${props.image}`
    : 'https://nickrosh.com/static/images/home-opt.jpg'
...
export async function getStaticProps({ params }) {
  try {
    ...
    const base = isProd ? 'https://nickrosh.com' : 'http://localhost:3000'
